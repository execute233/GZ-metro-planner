...
- 所有选择已有站点的字段使用独立搜索层：在字段上按 Enter 打开，输入中文、拼音或首字母筛选（英文大小写不敏感），↑↓ 选择候选，Enter 按站点 ID 选定并回填全名。列表显示所属线路；删除页会提示仍被区间引用的站点。搜索层 Esc 只放弃本次选择并返回原字段，保留其他表单内容；选定不会立即执行删除。新增站名和修改后的站名仍可自由输入。
